...
def leer_recibos(path_pdf):
    ...
    for i in range(0, len(bloques), 2):
        ...
        nombre_match = re.match(r'^([A-ZÑÁÉÍÓÚÜ, ]+)', texto.strip())
        nombre = nombre_match.group(1).strip() if nombre_match else 'SIN NOMBRE'

        subroles = re.split(r'Rol:\s*(\d+)', texto)

        for j in range(1, len(subroles), 2):
            rol = subroles[j]
            cuerpo = subroles[j+1]
            ...
            if not recibo['mes']:
                mes_match = re.search(r'(Enero|Febrero|Marzo|Abril|Mayo|Junio|Julio|Agosto|Septiembre|Octubre|Noviembre|Diciembre)\s+\d{4}', cuerpo, re.IGNORECASE)
                if mes_match:
            ...
            item_pattern = re.compile(
                r"^(?P<tipo>DV|RT)\s+(?P<codigo>\d{6,7})\s+(?P<descripcion>.+?)\s+(?P<monto>\d{1,3}(?:\.\d{3})*,\d{2})$"
            )

            for linea in lineas:
                linea = linea.strip()
                match = item_pattern.match(linea)
                if match:
                    codigo = match.group("codigo")
                    descripcion = match.group("descripcion").strip()
                    monto = limpiar_monto(match.group("monto"))
                    tipo = 'rem' if match.group("tipo") == "DV" else 'desc'
