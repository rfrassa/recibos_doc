...
def leer_recibos(path_pdf):
    ...
    for i in range(0, len(bloques), 2):
        ...
        patrones = {
            ...
            'liquido': r'Liq(?:uido)?\.?\s+(?:Pesos|Letras)?\s*:?[\s$]*([\d.,]+)'
        }
        ...
        # Fallback: buscar mes en contenido o usar nombre de archivo
        if not recibo['mes']:
            mes_match = re.search(
        ...
        # Extraer conceptos por línea
        for idx, linea in enumerate(lineas):
            linea = linea.strip()
            match_completo = re.match(r'^(\d{5,7})\s+(.*?)\s+([\d.,]+)$', linea)
            if match_completo:
                codigo = match_completo.group(1)
                descripcion = match_completo.group(2).strip()
                monto = limpiar_monto(match_completo.group(3))
            else:
                match_parcial = re.match(r'^(\d{5,7})\s+(.*)', linea)
                if match_parcial and idx + 1 < len(lineas):
                    codigo = match_parcial.group(1)
                    descripcion = match_parcial.group(2).strip()
                    monto_linea = lineas[idx + 1].strip()
                    monto_match = re.match(r'^\$?\s*([\d.,]+)$', monto_linea)
                    monto = limpiar_monto(monto_match.group(1)) if monto_match else 0
                else:
                    continue

            if "DV" in linea or codigo.startswith("1"):
                tipo = 'rem'
            elif "RT" in linea or codigo.startswith("6"):
                tipo = 'desc'
            else:
                tipo = 'otro'
...
#         # Fallback: buscar mes manualmente si no se detectó
#         if not recibo['mes']:
#             mes_match = re.search(
#                 r'(Enero|Febrero|Marzo|Abril|Mayo|Junio|Julio|Agosto|Septiembre|Octubre|Noviembre|Diciembre)\s+\d{4}',
#                 bloque, re.IGNORECASE
